...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Form->postLink(
                __('Delete'),
                ['action' => 'delete', $mockVivaLog->id],
                ['confirm' => __('Are you sure you want to delete # {0}?', $mockVivaLog->id)]
            )
        ?></li>
...
<div class="mockVivaLogs form large-9 medium-8 columns content">
    <?= $this->Form->create($mockVivaLog) ?>
    <fieldset>
        <legend><?= __('Edit Mock Viva Log') ?></legend>
        <?php
